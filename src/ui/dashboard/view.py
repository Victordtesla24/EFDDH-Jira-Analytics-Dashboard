...
def calculate_metrics(data: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
    ...
    if data is not None and not data.empty:
        ...
        if "Story Points" in data.columns:
            ...
            metrics["current"]["story_points"] = completed_issues["Story Points"].fillna(0).sum()
            
            # Calculate completion rate
            if metrics["current"]["total_issues"] > 0:
                completion_rate = (metrics["current"]["completed"] / metrics["current"]["total_issues"]) * 100
        ...
        if "Sprint" in data.columns and not data["Sprint"].empty:
            ...
            if len(sprints) > 0:
                try:
                    ...
                    if sprint_numbers:
                        current_sprint = max(sprint_numbers.keys(), key=lambda s: sprint_numbers[s])
